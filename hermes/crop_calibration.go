...
func isValidCropParameter(param string) bool {
	if param == "MAXAMAX" || param == "MINTMP" || param == "WUMAXPF" || param == "VELOC" || param == "YIFAK" ||
		...
		param == "DRYSWELL" || param == "LUKRIT" || param == "LAIFKT" || param == "WGMAX" {
		return true
	}
	return false
}
...
func (cropOW *CropOverwrite) OverwriteCropParameters(cropFile string, g *GlobalVarsMain) {
	if cropOW.CropFile != filepath.Base(cropFile) {
		return
	}
	if !cropOW.isValidCropOverwrite() {
		return
	}
...
func (cropOW *CropOverwrite) isValidCropOverwrite() bool {
	if cropOW.CropFile == "" {
		return false
	}
	for key, value := range cropOW.BaseFloatParameters {
