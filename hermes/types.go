...
type GlobalVarsMain struct {
	...
	EINT                 [300]float64
	TILART               [200]int
	TilBDdec             []float64   // decrease bulk density
	TilRecompacting      []float64   // rückverdichtung / reverse decompacting factor
	RecompactingPerLayer [21]float64 // rückverdichtung / reverse decompacting factor per layer
	EINTE                [201]int    //(0:200)
	DGART                [300]string
	NDIR                 [300]float64
	...
	MINAOS                  [10]float64 //  NAOS for each layer to mineralization depth
	MINFOS                  [10]float64 // NFOS for each layer to mineralization depth
	CA                      [21]float64
	NDG                     DualType
	MZ                      int
	NBR                     int
	...
	TillagePoreSpace       bool    // activate tillage pore space function
	TillagePoreSpaceFactor float64 // factor for tillage pore space function
	CNRAT1                 float64
	PERG                   float64
	ETREL                  float64
	MAXHMOI                [300]float64
